...
object GameData {
    ...
    private val META = listOf(
        ...
        CategoryMeta("Special", R.color.Violet, 3, "A player may use this once, whenever they want", "tiles/special.txt"),
    ...
    fun categories(context: Context): List<Category> = META.map { m ->
        Category(m.name, m.colorRes, m.defaultWeight, m.info, loadPrompts(context, m.asset), m.isUltra)
    }

    private fun loadPrompts(context: Context, asset: String): List<String> = try {
